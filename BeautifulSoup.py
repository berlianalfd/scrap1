...
for publish in obj.find_all('div',class_='conten1'):
    data.append({"judul":publish.find('h2').text,"kategori":publish.find('a').text,"waktu_publish":publish.find('div',class_='date').text,"waktu_scrapping":now.strftime("%Y-%m-%d %H-:%M:%S")})
